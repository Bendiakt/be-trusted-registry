...
const MISSION_FIXTURE = {
  id: 20, company_name: 'Delta SA', company_id: 10,
  location: 'Dakar, SN', type: 'on_site',
  fee_usd: 600, status: 'available',
  pac_tier_required: 'S1', pac_name: null,
  description: 'Supply chain audit', created_at: '2026-05-01T00:00:00Z',
}

test.describe('Admin — missions tab', () => {
  test.beforeEach(async ({ page }) => {
    test.setTimeout(45000)
    await stubApi(page)
    await page.goto('/login')
  ...
  test('missions tab is reachable and shows title', async ({ page }) => {
    await page.goto('/admin')
    const missionsTab = page.getByRole('button', { name: /missions/i }).first()
    await expect(missionsTab).toBeVisible({ timeout: 5000 })
    await missionsTab.click()
    // "Nouvelle mission" create button is always visible in missions tab
    await expect(page.getByRole('button', { name: /Nouvelle mission/i })).toBeVisible({ timeout: 5000 })
  })

  test('"Nouvelle mission" button opens create modal', async ({ page }) => {
    await page.goto('/admin')
    await page.getByRole('button', { name: /missions/i }).first().click()
    await page.getByRole('button', { name: /Nouvelle mission/i }).click()
    // Modal heading
    await expect(page.getByText(/Créer une mission/i)).toBeVisible({ timeout: 3000 })
  })

  test('mission creation sends POST /api/admin/missions', async ({ page }) => {
    ...
    await page.route('**/api/admin/missions', (route) => {
      if (route.request().method() === 'POST') {
        ...
        return route.fulfill({
          ...
          body: JSON.stringify({ mission: { ...MISSION_FIXTURE, id: 21 } }),
        })
      }
      return route.fulfill({
        status: 200, contentType: 'application/json',
        body: JSON.stringify({ data: [MISSION_FIXTURE], pagination: { page: 1, limit: 50, total: 1, pages: 1 } }),
      })
    })
    ...
    // Fill title (required — button disabled when empty)
    const titleInput = page.getByPlaceholder(/Audit Fournisseur/i)
    await expect(titleInput).toBeVisible({ timeout: 3000 })
    await titleInput.fill('Test Audit Mission')

    // Fill fee_usd (number input)
    const feeInput = page.locator('input[type="number"]').first()
    await expect(feeInput).toBeVisible({ timeout: 3000 })
    await feeInput.fill('600')

    // Submit
    const createBtn = page.getByRole('button', { name: /Créer/i }).last()
  ...
  test('"Assigner" button opens assign modal and PATCH is sent', async ({ page }) => {
    let assignCalled = false
    // Override missions stub to return one available mission
    await page.route('**/api/admin/missions**', (route) => {
      if (route.request().method() === 'POST') {
        return route.fulfill({ status: 201, contentType: 'application/json', body: JSON.stringify({ mission: MISSION_FIXTURE }) })
      }
      return route.fulfill({
        status: 200, contentType: 'application/json',
        body: JSON.stringify({ data: [MISSION_FIXTURE], pagination: { page: 1, limit: 50, total: 1, pages: 1 } }),
    ...
    await page.route(`**/api/admin/missions/20/assign`, (route) => {
      assignCalled = true
      return route.fulfill({
        status: 200, contentType: 'application/json',
        body: JSON.stringify({ mission: { ...MISSION_FIXTURE, status: 'assigned', pac_name: 'Sophie D.' }, agent: { name: 'Sophie D.' } }),
      })
    })
    ...
    // "Delta SA" should be visible from the fixture
    await expect(page.getByText('Delta SA')).toBeVisible({ timeout: 5000 })

    // Click "Assigner" button (only shows for available missions)
    const assignBtn = page.getByRole('button', { name: /Assigner/i }).first()
    await expect(assignBtn).toBeVisible({ timeout: 5000 })
    await assignBtn.click()

    // Modal opens
    await expect(page.getByText(/Assigner la mission #20/i)).toBeVisible({ timeout: 3000 })

    // Fill user ID and click confirm
    const userIdInput = page.locator('input[type="number"]').first()
    await userIdInput.fill('5')
    const confirmBtn = page.getByRole('button', { name: /✓ Assigner/i })
    await expect(confirmBtn).toBeVisible({ timeout: 3000 })
    await confirmBtn.click()

    await expect(async () => { expect(assignCalled).toBe(true) }).toPass({ timeout: 5000 })
  })

  test('mission cancel button calls PATCH /api/admin/missions/:id/cancel', async ({ page }) => {
    let cancelCalled = false
    // Override: one assigned mission (cancel button shows for non-completed/non-cancelled)
    const assignedMission = { ...MISSION_FIXTURE, status: 'assigned', pac_name: 'Sophie D.' }
    // Broad missions stub (GET + POST)
    await page.route('**/api/admin/missions**', (route) => {
      if (route.request().method() === 'POST') {
        return route.fulfill({ status: 201, contentType: 'application/json', body: JSON.stringify({ mission: assignedMission }) })
      }
      return route.fulfill({
        status: 200, contentType: 'application/json',
        body: JSON.stringify({ data: [assignedMission], pagination: { page: 1, limit: 50, total: 1, pages: 1 } }),
      })
    })
    // Specific cancel route registered AFTER broad route (LIFO — takes priority)
    await page.route(`**/api/admin/missions/${MISSION_FIXTURE.id}/cancel`, (route) => {
      cancelCalled = true
      return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ ok: true }) })
    })
    ...
    // cancelMission() uses window.confirm — accept it so the PATCH is sent
    page.on('dialog', d => d.accept())

    // Click the "✕ Annuler" button on the mission card
    const cancelBtn = page.getByRole('button', { name: /✕ Annuler/i }).first()
    await expect(cancelBtn).toBeVisible({ timeout: 5000 })
    await cancelBtn.click()

    await expect(async () => { expect(cancelCalled).toBe(true) }).toPass({ timeout: 5000 })
  })
})
...
test.describe('Admin — audit log tab', () => {
  ...
  test('audit log tab is reachable from admin panel', async ({ page }) => {
    await page.goto('/admin')
    ...
    await expect(page.getByPlaceholder(/Search action/i)).toBeVisible({ timeout: 5000 })
  })

  test('audit log shows entries from API', async ({ page }) => {
    await page.route('**/api/admin/audit-log**', (route) =>
      route.fulfill({
        status: 200, contentType: 'application/json',
        body: JSON.stringify({
          data: [
            { id: 1, action: 'company_certified', resource: 'companies', user_name: 'Super Admin', user_email: 'admin@mydd.work', ip_address: '127.0.0.1', created_at: '2026-05-30T10:00:00Z' },
          ],
          pagination: { page: 1, limit: 50, total: 1, pages: 1 },
        }),
      }),
    ...
    // Audit entry action and email should appear
    await expect(page.getByText('company_certified')).toBeVisible({ timeout: 5000 })
    await expect(page.getByText('admin@mydd.work').first()).toBeVisible({ timeout: 5000 })
  })
})
