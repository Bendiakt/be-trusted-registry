...
const PAC_TIER_NAMES = { S2: 'PAC Certified (S2)', S3: 'PAC Senior (S3)' }
const PAC_TIER_PRICES = { S2: '399', S3: '799' }

const sendPacMembershipConfirmation = async ({ email, agentName, tier, membershipExpires }) => {
  if (!email) return
  const tierName  = PAC_TIER_NAMES[tier]  || tier
  const tierPrice = PAC_TIER_PRICES[tier] || '?'
...
<body style="font-family:system-ui,sans-serif;background:#0a0a0a;color:#f5f5f5;padding:40px 16px;margin:0">
  <table style="max-width:560px;margin:0 auto;background:#111;border:1px solid #2a2a2a;border-radius:8px;overflow:hidden">
    ...
    <tr><td style="padding:32px">
      ...
      <p style="color:#aaa;line-height:1.6;margin:0 0 24px">
        Hello ${agentName ? `<strong style="color:#f5f5f5">${agentName}</strong>` : 'there'}, your <strong style="color:#C9A84C">${tierName}</strong> annual membership has been received.
        Your application is now under review by the B&amp;E team — you will be notified once approved.
      </p>
      <table style="width:100%;border-collapse:collapse;font-size:0.95rem;margin-bottom:24px">
        <tr>
          <td style="padding:8px 0;color:#aaa;border-bottom:1px solid #222">Membership tier</td>
          <td style="padding:8px 0;text-align:right;color:#C9A84C;border-bottom:1px solid #222"><strong>${tierName}</strong></td>
        </tr>
        <tr>
          <td style="padding:8px 0;color:#aaa;border-bottom:1px solid #222">Annual fee</td>
          <td style="padding:8px 0;text-align:right;color:#f5f5f5;border-bottom:1px solid #222"><strong>$${tierPrice} USD</strong></td>
        </tr>
        <tr>
          <td style="padding:8px 0;color:#aaa">Valid until</td>
          <td style="padding:8px 0;text-align:right;color:#f5f5f5"><strong>${expiresStr}</strong></td>
        </tr>
      </table>
      <p style="color:#aaa;font-size:0.85rem;margin:0">
        Questions? Contact <a href="mailto:support@mydd.work" style="color:#C9A84C">support@mydd.work</a>
      </p>
    </td></tr>
    <tr><td style="padding:16px 32px;background:#0d0d0d;font-size:0.75rem;color:#555;text-align:center">
      B&amp;E Consult FZCO &bull; Dubai, UAE &bull; <a href="https://mydd.work" style="color:#555">mydd.work</a>
...
module.exports = { sendPaymentConfirmation, sendWelcome, sendPasswordReset, sendMissionAssigned, sendMissionCompleted, sendCertGranted, sendEmailVerification, sendRenewalReminder, sendCertExpired, sendOnboardingD1, sendOnboardingD3, sendSupervisionTaskReminder, sendPacMembershipConfirmation }
